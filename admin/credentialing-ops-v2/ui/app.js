/**
 * EnnHealth Credentialing Ops v2 — App Controller
 *
...
import { APPLICATION_STATUSES, LICENSE_STATUSES, ACTIVITY_LOG_TYPES } from '../data/schema.js';
import { PAYER_CATALOG, getPayerById } from '../data/payers.js';
import { STATES, getStateName, getStatePop, US_TOTAL_POP } from '../data/states.js';
import { TELEHEALTH_POLICIES } from '../data/telehealth-policies.js';
import { DEFAULT_STRATEGIES } from '../data/strategies.js';
import { DEFAULT_ORGANIZATION, DEFAULT_PROVIDER, DEFAULT_LICENSES } from '../data/providers.js';
import {
  transitionApplication, getAvailableTransitions,
  getOverdueFollowups, getUpcomingFollowups, getFollowupsForApplication,
  completeFollowup, getEscalationCandidates, getAgedApplications,
} from '../core/workflow.js';
import {
  generateBatch, commitBatch, summarizeBatch,
} from '../core/batch-generator.js';
import {
  importFromV1LocalStorage, importFromCSV, previewImport, exportV2Data,
  importFromV1Cloud, previewV1CloudImport,
} from '../core/migration.js';
import {
  generateEmail, generateEmailForApplication, generateExpansionEmails, getTemplateList,
} from '../core/email-generator.js';
import {
  getCaqhConfig, saveCaqhConfig, isCaqhConfigured,
  getRosterStatus, getProviderStatus, getProviderStatusByNPI,
  getAttestationStatus, getProviderProfile, batchStatusCheck,
  addToRoster, getCaqhTracking, saveCaqhTracking, updateTrackingFromResult,
} from '../core/caqh-api.js';
import {
  lookupNPI, searchProviders, searchByTaxonomy,
  searchTaxonomyCodes, TAXONOMY_CODES,
} from '../core/taxonomy-api.js';
...
export async function initApp() {
  ...
  // Initialize store (fetches from Google Sheets or falls back to localStorage)
  try {
    await store.init();
  } catch (err) {
  ...
  try {
    ...
    console.error('Store flush error:', err);
  }
  ...
  store.onSyncStatus((status, error) => {
...
function seedDefaults() {
  // Seed default org + provider + licenses if empty
  if (store.getAll('organizations').length === 0) {
    store.add('organizations', 'organization', DEFAULT_ORGANIZATION);
  }
  if (store.getAll('providers').length === 0) {
    store.add('providers', 'provider', DEFAULT_PROVIDER);
  }
  if (store.getAll('licenses').length === 0) {
    DEFAULT_LICENSES.forEach(lic => {
      store.add('licenses', 'license', {
        ...lic,
  ...
  // Seed telehealth policies from static data if Sheets collection is empty
  if (store.getAll('telehealth_policies').length === 0 && TELEHEALTH_POLICIES.length > 0) {
    const now = new Date().toISOString();
    const policyRecords = TELEHEALTH_POLICIES.map(p => ({
      ...p,
      id: `thp_${p.state}`,
      createdAt: now,
      updatedAt: now,
    }));
    store.bulkReplace('telehealth_policies', policyRecords);
    console.log(`Seeded ${policyRecords.length} telehealth policies to store`);
  }
}
...
function navigateTo(page) {
  ...
  const pendingTasks = store.getAll('tasks').filter(t => !t.completed && t.dueDate && t.dueDate <= today).length;
  const taskBadge = document.getElementById('task-badge');
  if (taskBadge) {
    taskBadge.textContent = pendingTasks;
    taskBadge.style.display = pendingTasks > 0 ? 'inline' : 'none';
  }
  ...
  switch (page) {
    ...
    case 'users':
      ...
      pageSubtitle.textContent = 'Create and manage platform users';
      pageActions.innerHTML = '<button class="btn btn-gold" onclick="window.app.openCreateUserModal()">+ Create User</button>' + printBtn;
      renderUsersPage();
      break;
    case 'send-email':
      pageTitle.textContent = 'Send Email';
      pageSubtitle.textContent = 'Compose and send emails to providers, payers, and staff';
      pageActions.innerHTML = '';
      renderSendEmailPage();
      break;
    case 'invoices':
      pageTitle.textContent = 'Invoices';
      pageSubtitle.textContent = 'Create and manage credentialing service invoices';
      pageActions.innerHTML = '<button class="btn btn-gold" onclick="window.app.openCreateInvoiceModal()">+ Create Invoice</button>' + printBtn;
      renderInvoicesPage();
      break;
    default:
...
function renderDashboard() {
  ...
  const escalations = getEscalationCandidates();
  ...
  body.innerHTML = `
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:16px;">
      ...
      ${(() => {
        const allApps = store.getAll('applications');
...
function renderFollowups() {
  const body = document.getElementById('page-body');
  const overdue = getOverdueFollowups();
  const upcoming = getUpcomingFollowups(14);
  const allOpen = store.getAll('followups').filter(f => !f.completedDate);
  const completed = store.getAll('followups').filter(f => f.completedDate)
...
function getLivePolicies() {
  const fromStore = store.getAll('telehealth_policies');
  return fromStore.length > 0 ? fromStore : TELEHEALTH_POLICIES;
}
...
function renderEmailGenerator() {
  ...
  const apps = store.getAll('applications').filter(a => a.status !== 'approved' && a.status !== 'denied');
        ...
        <div class="card-body">
          <div class="form-group">
            ...
            <select class="form-control" id="email-expansion-states" multiple size="10" style="min-height:180px;">
              ${(() => {
                const licensedCodes = new Set(store.getAll('licenses').filter(l => l.status === 'active').map(l => l.state));
      ...
      <table>
        ...
        <tbody>
          ${providers.map(p => {
            ...
            return `
              <tr style="cursor:pointer;" onclick="window.app.viewProvider('${p.id}')">
                <td>
                  <strong>${escHtml(p.firstName)} ${escHtml(p.lastName)}</strong>
                  ${p.credentials ? `<br><span class="text-sm text-muted">${escHtml(p.credentials)}</span>` : ''}
                </td>
                <td><code style="color:var(--brand-700);">${p.npi || '—'}</code></td>
                <td class="text-sm">${escHtml(p.specialty) || '—'}</td>
                <td class="text-sm">${org ? `<a href="#" onclick="event.stopPropagation();window.app.viewOrg('${org.id}')">${escHtml(org.name)}</a>` : '—'}</td>
                <td><strong>${activeLic}</strong><span class="text-muted">/${provLicenses.length}</span></td>
                <td>${provApps.length}</td>
                <td><span class="badge badge-${p.active !== false ? 'active' : 'inactive'}">${p.active !== false ? 'Active' : 'Inactive'}</span></td>
                <td onclick="event.stopPropagation();">
                  <button class="btn btn-sm" onclick="window.app.editProvider('${p.id}')">Edit</button>
                  <button class="btn btn-sm" style="color:var(--red);" onclick="window.app.deleteProvider('${p.id}')">Del</button>
                </td>
              </tr>`;
          }).join('')}
        </tbody>
...
let _detailProviderId = null;

function renderProviderDetail(providerId) {
  _detailProviderId = providerId;
  const body = document.getElementById('page-body');
  const p = store.getById('providers', providerId);
  if (!p) { body.innerHTML = '<div class="empty-state"><h3>Provider not found</h3></div>'; return; }

  const licenses = store.getAll('licenses').filter(l => l.providerId === providerId);
  const apps = store.getAll('applications').filter(a => a.providerId === providerId);
  const logs = store.getAll('activity_logs').filter(l => apps.some(a => a.id === l.applicationId));
  const orgs = store.getAll('organizations');
  const org = orgs.find(o => o.id === p.orgId);
  const activeLic = licenses.filter(l => l.status === 'active').length;
  const pendingLic = licenses.filter(l => l.status === 'pending').length;
  const expiredLic = licenses.filter(l => l.status === 'expired').length;
  const licensedStates = [...new Set(licenses.map(l => l.state))];
  const today = new Date().toISOString().split('T')[0];
  const expiringLic = licenses.filter(l => l.expirationDate && l.expirationDate > today && l.expirationDate < new Date(Date.now() + 90 * 86400000).toISOString().split('T')[0]);
  const approvedApps = apps.filter(a => a.status === 'approved').length;
  const pendingApps = apps.filter(a => ['submitted', 'in_review', 'pending_info'].includes(a.status)).length;
  const estRevenue = apps.filter(a => a.status === 'approved').reduce((s, a) => s + (a.estMonthlyRevenue || 0), 0);

  // Count provider profile records
  const eduCount = store.getAll('provider_education').filter(e => e.providerId === providerId).length;
  const boardCount = store.getAll('board_certifications').filter(e => e.providerId === providerId).length;
  const workCount = store.getAll('work_history').filter(e => e.providerId === providerId).length;
  const malCount = store.getAll('malpractice_policies').filter(e => e.providerId === providerId).length;
  const cmeCount = store.getAll('provider_cme').filter(e => e.providerId === providerId).length;
  const refCount = store.getAll('provider_references').filter(e => e.providerId === providerId).length;
  const docCount = store.getAll('provider_documents').filter(e => e.providerId === providerId).length;

  const pageTitle = document.getElementById('page-title');
  const pageSubtitle = document.getElementById('page-subtitle');
  const pageActions = document.getElementById('page-actions');
  if (pageTitle) pageTitle.textContent = `${p.firstName} ${p.lastName}`;
  if (pageSubtitle) pageSubtitle.textContent = p.credentials || p.specialty || 'Provider Detail';
  if (pageActions) pageActions.innerHTML = `
    <button class="btn btn-sm" onclick="window.app.navigateTo('providers')">← Back</button>
    <button class="btn btn-sm" onclick="window.app.editProvider('${p.id}')">Edit Provider</button>
    <button class="btn btn-sm btn-gold" onclick="window.app.openLicenseModal(null, '${p.id}')">+ Add License</button>
  `;
  ...
  body.innerHTML = `
    ...
    <div class="card" style="border-top:3px solid var(--brand-600);margin-bottom:20px;">
      <div class="card-body">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:16px;">
          <div>
            <div style="font-size:22px;font-weight:800;color:var(--gray-900);">${escHtml(p.firstName)} ${escHtml(p.lastName)}${p.credentials ? ', ' + escHtml(p.credentials) : ''}</div>
            <div style="display:flex;gap:24px;flex-wrap:wrap;margin-top:8px;font-size:13px;color:var(--gray-600);">
              <div>NPI: <strong style="color:var(--brand-700);">${p.npi || '—'}</strong></div>
              <div>Taxonomy: <strong>${p.taxonomy || '—'}</strong></div>
              <div>Specialty: <strong>${escHtml(p.specialty) || '—'}</strong></div>
              ${org ? `<div>Org: <a href="#" onclick="window.app.viewOrg('${org.id}')">${escHtml(org.name)}</a></div>` : ''}
            </div>
            <div style="display:flex;gap:24px;flex-wrap:wrap;margin-top:6px;font-size:13px;color:var(--gray-600);">
              ${p.email ? `<div>Email: <a href="mailto:${escAttr(p.email)}">${escHtml(p.email)}</a></div>` : ''}
              ${p.phone ? `<div>Phone: ${escHtml(p.phone)}</div>` : ''}
            </div>
          </div>
          <span class="badge badge-${p.active !== false ? 'active' : 'inactive'}" style="font-size:14px;padding:6px 16px;">${p.active !== false ? 'Active' : 'Inactive'}</span>
        </div>
      </div>
    </div>
    ...
    <div class="stats-grid" style="grid-template-columns:repeat(auto-fit,minmax(140px,1fr));margin-bottom:20px;">
      <div class="stat-card"><div class="label">Licenses</div><div class="value">${licenses.length}</div><div class="sub">${activeLic} active, ${pendingLic} pending${expiredLic ? ', ' + expiredLic + ' expired' : ''}</div></div>
      <div class="stat-card"><div class="label">Licensed States</div><div class="value blue">${licensedStates.length}</div></div>
      <div class="stat-card"><div class="label">Applications</div><div class="value">${apps.length}</div><div class="sub">${approvedApps} approved, ${pendingApps} pending</div></div>
      <div class="stat-card"><div class="label">Est. Monthly Rev</div><div class="value green">$${estRevenue.toLocaleString()}</div></div>
      ${expiringLic.length > 0 ? `<div class="stat-card" style="border-top:3px solid var(--red);"><div class="label">Expiring Soon</div><div class="value" style="color:var(--red);">${expiringLic.length}</div><div class="sub">Within 90 days</div></div>` : ''}
    </div>

    <!-- Tabs -->
    <div class="tabs" style="margin-bottom:16px;flex-wrap:wrap;">
      <button class="tab active" onclick="window.app.provDetailTab(this, 'pd-licenses')">Licenses (${licenses.length})</button>
      <button class="tab" onclick="window.app.provDetailTab(this, 'pd-applications')">Applications (${apps.length})</button>
      <button class="tab" onclick="window.app.provDetailTab(this, 'pd-activity')">Activity Log (${logs.length})</button>
      <button class="tab" onclick="window.app.provDetailTab(this, 'pd-education')">Education (${eduCount})</button>
      <button class="tab" onclick="window.app.provDetailTab(this, 'pd-boards')">Boards (${boardCount})</button>
      <button class="tab" onclick="window.app.provDetailTab(this, 'pd-work-history')">Work History (${workCount})</button>
      <button class="tab" onclick="window.app.provDetailTab(this, 'pd-malpractice')">Malpractice (${malCount})</button>
      <button class="tab" onclick="window.app.provDetailTab(this, 'pd-cme')">CME (${cmeCount})</button>
      <button class="tab" onclick="window.app.provDetailTab(this, 'pd-references')">References (${refCount})</button>
      <button class="tab" onclick="window.app.provDetailTab(this, 'pd-documents')">Documents (${docCount})</button>
    </div>

    <!-- Licenses Tab -->
    <div id="pd-licenses">
      ${licenses.length === 0 ? '<div class="empty-state" style="padding:30px;"><p>No licenses. Click "+ Add License" to add one.</p></div>' : `
        <div class="table-wrap">
          <table>
            <thead><tr><th>State</th><th>License #</th><th>Type</th><th>Status</th><th>Issue Date</th><th>Expiration</th><th>Compact</th><th>Notes</th><th>Actions</th></tr></thead>
            <tbody>
              ${licenses.sort((a, b) => (a.state || '').localeCompare(b.state || '')).map(l => {
                const isExpiring = l.expirationDate && l.expirationDate > today && l.expirationDate < new Date(Date.now() + 90 * 86400000).toISOString().split('T')[0];
                const isExpired = l.expirationDate && l.expirationDate < today;
                return `
                  <tr${isExpiring ? ' style="background:var(--gold-50);"' : isExpired ? ' style="background:var(--red-50,#fef2f2);"' : ''}>
                    <td><strong>${getStateName(l.state)}</strong> (${l.state})</td>
                    <td><code>${escHtml(l.licenseNumber) || '—'}</code></td>
                    <td>${escHtml(l.licenseType) || '—'}</td>
                    <td><span class="badge badge-${l.status}">${l.status}</span></td>
                    <td class="text-sm">${l.issueDate || '—'}</td>
                    <td class="text-sm">${l.expirationDate || '—'}${isExpiring ? ' ⚠️' : ''}</td>
                    <td>${l.compactState ? '✓' : '—'}</td>
                    <td class="text-sm truncate" style="max-width:200px;">${escHtml(l.notes) || '—'}</td>
                    <td>
                      <button class="btn btn-sm" onclick="window.app.editLicense('${l.id}')">Edit</button>
                      <button class="btn btn-sm" style="color:var(--red);" onclick="window.app.deleteLicense('${l.id}');window.app.viewProvider('${p.id}');">Del</button>
                    </td>
                  </tr>`;
              }).join('')}
            </tbody>
    ...
    <!-- Applications Tab -->
    <div id="pd-applications" class="hidden">
      ${apps.length === 0 ? '<div class="empty-state" style="padding:30px;"><p>No applications for this provider.</p></div>' : `
        <div class="table-wrap">
          <table>
            <thead><tr><th>Payer</th><th>State</th><th>Type</th><th>Wave</th><th>Status</th><th>Submitted</th><th>Effective</th><th>Est Rev</th><th>Actions</th></tr></thead>
            <tbody>
              ${apps.sort((a, b) => (b.createdAt || '').localeCompare(a.createdAt || '')).map(a => `
                <tr>
                  <td><strong>${escHtml(a.payerName || '')}</strong></td>
                  <td>${getStateName(a.state)} (${a.state})</td>
                  <td>${a.type || '—'}</td>
                  ...
                  <td class="text-sm">${a.submittedDate || '—'}</td>
                  <td class="text-sm">${a.effectiveDate || '—'}</td>
                  <td>$${(a.estMonthlyRevenue || 0).toLocaleString()}</td>
                  <td>
                    <button class="btn btn-sm" onclick="window.app.editApplication('${a.id}')">Edit</button>
                    <button class="btn btn-sm" onclick="window.app.viewActivityLog('${a.id}')">Log</button>
                  </td>
                </tr>
              `).join('')}
            </tbody>
    ...
    <!-- Activity Log Tab -->
    <div id="pd-activity" class="hidden">
      ${logs.length === 0 ? '<div class="empty-state" style="padding:30px;"><p>No activity logs for this provider\'s applications.</p></div>' : `
        <div class="table-wrap">
          <table>
            <thead><tr><th>Date</th><th>Type</th><th>Application</th><th>From</th><th>To</th><th>Outcome</th></tr></thead>
            <tbody>
              ${logs.sort((a, b) => (b.loggedDate || '').localeCompare(a.loggedDate || '')).map(l => {
                const app = apps.find(a => a.id === l.applicationId);
                return `
                  <tr>
                    <td class="text-sm">${l.loggedDate || '—'}</td>
                    <td><span class="badge badge-${l.type === 'status_change' ? 'submitted' : 'active'}">${(l.type || '').replace(/_/g, ' ')}</span></td>
                    <td class="text-sm">${app ? escHtml(app.payerName || '') + ' — ' + (app.state || '') : '—'}</td>
                    <td class="text-sm">${l.statusFrom ? `<span class="badge badge-${l.statusFrom}">${l.statusFrom.replace(/_/g, ' ')}</span>` : '—'}</td>
                    <td class="text-sm">${l.statusTo ? `<span class="badge badge-${l.statusTo}">${l.statusTo.replace(/_/g, ' ')}</span>` : '—'}</td>
                    <td class="text-sm truncate" style="max-width:250px;">${escHtml(l.outcome) || '—'}</td>
                  </tr>`;
              }).join('')}
            </tbody>
          </table>
    ...
    <!-- Education Tab -->
    <div id="pd-education" class="hidden">
      ${renderProviderProfileTab(providerId, 'provider_education', 'Education', [
        { key: 'institution', label: 'Institution' },
        { key: 'degree', label: 'Degree' },
        { key: 'fieldOfStudy', label: 'Field of Study' },
        { key: 'educationType', label: 'Type', format: v => (v || '').replace(/_/g, ' ') },
        { key: 'startDate', label: 'Start' },
        { key: 'graduationDate', label: 'Graduation' },
        { key: 'isCompleted', label: 'Completed', format: v => v ? 'Yes' : 'No' },
      ])}
    </div>

    <!-- Board Certifications Tab -->
    <div id="pd-boards" class="hidden">
      ${renderProviderProfileTab(providerId, 'board_certifications', 'Board Certification', [
        ...
        { key: 'isLifetime', label: 'Lifetime', format: v => v ? 'Yes' : 'No' },
      ])}
    </div>

    <!-- Work History Tab -->
    <div id="pd-work-history" class="hidden">
      ${renderProviderProfileTab(providerId, 'work_history', 'Work History', [
        { key: 'employer', label: 'Employer' },
        { key: 'position', label: 'Position' },
        { key: 'startDate', label: 'Start' },
        { key: 'endDate', label: 'End' },
        { key: 'isCurrent', label: 'Current', format: v => v ? 'Yes' : '—' },
    ...
    <!-- Malpractice Tab -->
    <div id="pd-malpractice" class="hidden">
      ${renderProviderProfileTab(providerId, 'malpractice_policies', 'Malpractice Policy', [
        { key: 'carrierName', label: 'Carrier' },
        { key: 'policyNumber', label: 'Policy #' },
        { key: 'coverageType', label: 'Type', format: v => (v || '').replace(/_/g, ' ') },
        { key: 'perIncident', label: 'Per Incident' },
        { key: 'aggregate', label: 'Aggregate' },
        { key: 'effectiveDate', label: 'Effective' },
        { key: 'expirationDate', label: 'Expiration' },
        { key: 'hasTailCoverage', label: 'Tail', format: v => v ? 'Yes' : 'No' },
        { key: 'hasClaimsHistory', label: 'Claims', format: v => v ? 'Yes' : 'No' },
      ])}
    </div>

    <!-- CME Tab -->
    <div id="pd-cme" class="hidden">
      ${renderProviderProfileTab(providerId, 'provider_cme', 'CME', [
        { key: 'courseName', label: 'Course' },
        { key: 'providerOrg', label: 'Provider/Org' },
        { key: 'creditHours', label: 'Hours' },
        { key: 'creditType', label: 'Type', format: v => (v || '').toUpperCase() },
    ...
    <!-- References Tab -->
    <div id="pd-references" class="hidden">
      ${renderProviderProfileTab(providerId, 'provider_references', 'Reference', [
        { key: 'name', label: 'Name' },
    ...
    <!-- Documents Tab -->
    <div id="pd-documents" class="hidden">
      ${renderProviderProfileTab(providerId, 'provider_documents', 'Document', [
        { key: 'documentType', label: 'Type', format: v => (v || '').replace(/_/g, ' ') },
        { key: 'documentName', label: 'Name' },
        { key: 'status', label: 'Status', format: v => `<span class="badge badge-${v === 'verified' ? 'active' : v === 'expired' ? 'expired' : v === 'received' ? 'submitted' : 'pending'}">${v || '—'}</span>` },
...
// ─── Provider Profile Tab Helper ───

const PROVIDER_DOC_TYPES = [
  'cv_resume', 'medical_degree', 'board_certificate', 'state_license',
  'dea_certificate', 'cds_certificate', 'malpractice_coi', 'npi_confirmation',
  'caqh_attestation', 'immunization_records', 'tb_test', 'background_check',
  'drivers_license', 'passport', 'social_security_card', 'w9_form',
  'professional_photo', 'supervision_agreement', 'collaborative_agreement',
  'disclosure_form', 'other',
];

function renderProviderProfileTab(providerId, collection, label, columns) {
  const records = store.getAll(collection).filter(r => r.providerId === providerId);
  const collKey = collection; // for onclick refs
  return `
    <div style="margin-bottom:12px;">
      <button class="btn btn-gold btn-sm" onclick="window.app.openProvProfileForm('${providerId}', '${collKey}', '${label}')">+ Add ${label}</button>
    </div>
    ${records.length === 0 ? `<div class="empty-state" style="padding:30px;"><p>No ${label.toLowerCase()} records yet.</p></div>` : `
      <div class="table-wrap">
        <table>
          <thead><tr>${columns.map(c => `<th>${c.label}</th>`).join('')}<th>Actions</th></tr></thead>
          <tbody>
            ${records.sort((a, b) => (b.createdAt || '').localeCompare(a.createdAt || '')).map(r => `
              <tr>
                ${columns.map(c => {
                  const val = r[c.key];
                  const display = c.format ? c.format(val) : (escHtml(String(val || '')) || '—');
                  return `<td class="text-sm">${display}</td>`;
                }).join('')}
                <td>
                  <button class="btn btn-sm" onclick="window.app.editProvProfileRecord('${providerId}', '${collKey}', '${label}', '${r.id}')">Edit</button>
                  <button class="btn btn-sm" style="color:var(--red);" onclick="window.app.deleteProvProfileRecord('${providerId}', '${collKey}', '${r.id}')">Del</button>
                </td>
              </tr>
            `).join('')}
          </tbody>
...
function getProvProfileFormFields(collection) {
  const fields = {
    provider_education: [
      { key: 'institution', label: 'Institution *', type: 'text' },
      { key: 'degree', label: 'Degree', type: 'text', placeholder: 'e.g. DNP, MSN, MD' },
      { key: 'fieldOfStudy', label: 'Field of Study', type: 'text' },
      { key: 'educationType', label: 'Type', type: 'select', options: [
        { value: '', label: '— Select —' }, { value: 'medical_school', label: 'Medical School' },
        { value: 'residency', label: 'Residency' }, { value: 'fellowship', label: 'Fellowship' }, { value: 'other', label: 'Other' },
      ]},
    ...
    board_certifications: [
      { key: 'boardName', label: 'Board Name *', type: 'text', placeholder: 'e.g. ANCC, AANP' },
      { key: 'specialty', label: 'Specialty', type: 'text' },
      { key: 'certificateNumber', label: 'Certificate Number', type: 'text' },
      { key: 'initialDate', label: 'Initial Date', type: 'date' },
      { key: 'expirationDate', label: 'Expiration Date', type: 'date' },
      { key: 'isLifetime', label: 'Lifetime Certification', type: 'checkbox' },
    ],
    work_history: [
      ...
      { key: 'isCurrent', label: 'Currently Employed', type: 'checkbox' },
      { key: 'supervisorName', label: 'Supervisor Name', type: 'text' },
      { key: 'supervisorPhone', label: 'Supervisor Phone', type: 'text' },
    ],
    malpractice_policies: [
      { key: 'carrierName', label: 'Carrier Name *', type: 'text' },
      { key: 'policyNumber', label: 'Policy Number', type: 'text' },
      { key: 'coverageType', label: 'Coverage Type', type: 'select', options: [
        { value: '', label: '— Select —' }, { value: 'occurrence', label: 'Occurrence' }, { value: 'claims_made', label: 'Claims Made' },
      ]},
      { key: 'perIncident', label: 'Per Incident Limit', type: 'text', placeholder: 'e.g. $1,000,000' },
      { key: 'aggregate', label: 'Aggregate Limit', type: 'text', placeholder: 'e.g. $3,000,000' },
      { key: 'effectiveDate', label: 'Effective Date', type: 'date' },
      { key: 'expirationDate', label: 'Expiration Date', type: 'date' },
      { key: 'hasTailCoverage', label: 'Has Tail Coverage', type: 'checkbox' },
      { key: 'hasClaimsHistory', label: 'Has Claims History', type: 'checkbox' },
    ],
    provider_cme: [
      { key: 'courseName', label: 'Course Name *', type: 'text' },
      { key: 'providerOrg', label: 'Provider/Organization', type: 'text' },
      { key: 'creditHours', label: 'Credit Hours', type: 'number' },
      { key: 'creditType', label: 'Credit Type', type: 'select', options: [
        { value: '', label: '— Select —' }, { value: 'cme', label: 'CME' }, { value: 'ce', label: 'CE' }, { value: 'contact_hours', label: 'Contact Hours' },
      ]},
    ...
    provider_references: [
      ...
      { key: 'relationship', label: 'Relationship', type: 'text', placeholder: 'e.g. Clinical Supervisor' },
    ...
    provider_documents: [
      { key: 'documentType', label: 'Document Type *', type: 'select', options: [
        { value: '', label: '— Select —' },
        ...PROVIDER_DOC_TYPES.map(t => ({ value: t, label: t.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase()) })),
      ]},
...
// Map collection names to entity types for store.add
function collectionToEntityType(collection) {
  const map = {
    provider_education: 'provider_education',
    board_certifications: 'board_certification',
    work_history: 'work_history',
    malpractice_policies: 'malpractice_policy',
    provider_cme: 'provider_cme',
    provider_references: 'provider_reference',
    provider_documents: 'provider_document',
    exclusion_checks: 'exclusion_check',
    org_contacts: 'org_contact',
  };
  return map[collection] || collection;
}

function openProvProfileForm(providerId, collection, label, existingId) {
  const fields = getProvProfileFormFields(collection);
  const existing = existingId ? store.getById(collection, existingId) : null;
  const modal = document.getElementById('log-modal');
  document.getElementById('log-modal-title').textContent = existing ? `Edit ${label}` : `Add ${label}`;

  let formHtml = `<input type="hidden" id="ppf-id" value="${existingId || ''}">
    <input type="hidden" id="ppf-provider" value="${providerId}">
    <input type="hidden" id="ppf-collection" value="${collection}">`;

  for (const f of fields) {
    const val = existing ? (existing[f.key] ?? '') : '';
    if (f.type === 'select') {
      formHtml += `<div class="form-group"><label>${f.label}</label><select class="form-control" id="ppf-${f.key}">
        ${f.options.map(o => `<option value="${o.value}" ${val === o.value ? 'selected' : ''}>${o.label}</option>`).join('')}
      </select></div>`;
    } else if (f.type === 'checkbox') {
      formHtml += `<div class="form-group"><label style="display:flex;align-items:center;gap:8px;cursor:pointer;">
        <input type="checkbox" id="ppf-${f.key}" ${val ? 'checked' : ''} style="width:16px;height:16px;"> ${f.label}
      </label></div>`;
    } else {
      formHtml += `<div class="form-group"><label>${f.label}</label><input type="${f.type}" class="form-control" id="ppf-${f.key}" value="${escAttr(String(val))}" ${f.placeholder ? 'placeholder="' + escAttr(f.placeholder) + '"' : ''}></div>`;
    }
  }

  formHtml += `<div style="margin-top:16px;display:flex;gap:10px;justify-content:flex-end;">
    <button class="btn" onclick="window.closeLogModal()">Cancel</button>
    <button class="btn btn-primary" onclick="window.app.saveProvProfileRecord()">Save ${label}</button>
  </div>`;

  document.getElementById('log-modal-body').innerHTML = formHtml;
  modal.classList.add('active');
}

function saveProvProfileRecord() {
  const id = document.getElementById('ppf-id')?.value;
  const providerId = document.getElementById('ppf-provider')?.value;
  const collection = document.getElementById('ppf-collection')?.value;
  const fields = getProvProfileFormFields(collection);
  const data = { providerId };

  for (const f of fields) {
    const el = document.getElementById(`ppf-${f.key}`);
    if (!el) continue;
    if (f.type === 'checkbox') {
  ...
  if (id) {
    ...
    const result = store.add(collection, entityType, data);
    if (!result.success) { showToast('Error: ' + result.errors.join(', ')); return; }
...
function deleteProvProfileRecord(providerId, collection, recordId) {
...
function renderExclusionScreening() {
  const body = document.getElementById('page-body');
  const providers = store.getAll('providers');
  const checks = store.getAll('exclusion_checks');

  const getLastCheck = (providerId) => {
    const provChecks = checks.filter(c => c.providerId === providerId);
    if (provChecks.length === 0) return null;
    return provChecks.sort((a, b) => (b.checkDate || '').localeCompare(a.checkDate || ''))[0];
  };

  const resultBadge = (result) => {
    if (!result) return '<span class="badge badge-pending" style="background:#f1f5f9;color:#64748b;">Never Checked</span>';
    if (result === 'clear') return '<span class="badge badge-active">Clear</span>';
    if (result === 'excluded') return '<span class="badge badge-expired" style="background:#fee2e2;color:#991b1b;">EXCLUDED</span>';
    return '<span class="badge badge-pending" style="background:#fef3c7;color:#92400e;">Error</span>';
  };

  const clearCount = providers.filter(p => { const c = getLastCheck(p.id); return c?.result === 'clear'; }).length;
  const neverCount = providers.filter(p => !getLastCheck(p.id)).length;
  const excludedCount = providers.filter(p => { const c = getLastCheck(p.id); return c?.result === 'excluded'; }).length;

  body.innerHTML = `
    <div class="stats-grid" style="grid-template-columns:repeat(4,1fr);margin-bottom:20px;">
      <div class="stat-card"><div class="label">Providers</div><div class="value">${providers.length}</div></div>
      <div class="stat-card"><div class="label">Clear</div><div class="value green">${clearCount}</div></div>
      <div class="stat-card"><div class="label">Never Checked</div><div class="value" style="color:var(--gray-500);">${neverCount}</div></div>
      <div class="stat-card"><div class="label">Excluded</div><div class="value red">${excludedCount}</div></div>
    </div>

    <div class="card">
      <div class="card-header">
        <h3>Provider Exclusion Status</h3>
      </div>
      <div class="card-body" style="padding:0;">
        ${providers.length === 0 ? '<div class="empty-state" style="padding:30px;"><p>No providers to screen.</p></div>' : `
          <table>
            <thead><tr><th>Provider</th><th>NPI</th><th>Last Check</th><th>Result</th><th>Source</th><th>Actions</th></tr></thead>
            <tbody>
              ${providers.map(p => {
                const lastCheck = getLastCheck(p.id);
                return `
                  <tr>
                    <td><strong>${escHtml(p.firstName)} ${escHtml(p.lastName)}</strong></td>
                    <td><code>${p.npi || '—'}</code></td>
                    <td class="text-sm">${lastCheck?.checkDate || '—'}</td>
                    <td>${resultBadge(lastCheck?.result)}</td>
                    <td class="text-sm">${escHtml(lastCheck?.source || '') || '—'}</td>
                    <td>
                      <button class="btn btn-sm" onclick="window.app.screenProvider('${p.id}')">Screen</button>
                      <button class="btn btn-sm" onclick="window.app.manualExclusionResult('${p.id}')">Manual Entry</button>
                    </td>
                  </tr>`;
              }).join('')}
            </tbody>
...
function renderOrganizations() {
  ...
  body.innerHTML = `
    <div class="stats-grid" style="grid-template-columns:repeat(4,1fr);">
      <div class="stat-card"><div class="label">Organizations</div><div class="value">${orgs.length}</div></div>
      <div class="stat-card"><div class="label">Total Providers</div><div class="value blue">${providers.length}</div></div>
    ...
    ${orgs.map(o => {
      const orgProviders = providers.filter(p => p.orgId === o.id);
      const orgLicenses = licenses.filter(l => orgProviders.some(p => p.id === l.providerId));
      const orgApps = apps.filter(a => a.orgId === o.id || orgProviders.some(p => p.id === a.providerId));
      return `
        <div class="card" style="cursor:pointer;" onclick="window.app.viewOrg('${o.id}')">
          ...
          <div class="card-body">
            <div style="display:flex;gap:32px;flex-wrap:wrap;margin-bottom:12px;font-size:13px;color:var(--gray-600);">
              ...
              <div>Tax ID: <strong>${o.taxId || '—'}</strong></div>
              <div>Phone: ${escHtml(o.phone) || '—'}</div>
              <div>Email: ${escHtml(o.email) || '—'}</div>
              ${o.address ? `<div>${escHtml(o.address.city || '')}, ${escHtml(o.address.state || '')} ${escHtml(o.address.zip || '')}</div>` : ''}
            </div>
            <div style="display:flex;gap:16px;flex-wrap:wrap;">
              ...
              <div class="stat-card" style="flex:1;min-width:100px;"><div class="label">Licenses</div><div class="value blue">${orgLicenses.length}</div></div>
...
function renderOrgDetail(orgId) {
  _detailOrgId = orgId;
  const body = document.getElementById('page-body');
  const o = store.getById('organizations', orgId);
  if (!o) { body.innerHTML = '<div class="empty-state"><h3>Organization not found</h3></div>'; return; }

  const providers = store.getAll('providers').filter(p => p.orgId === orgId);
  const allLicenses = store.getAll('licenses');
  const allApps = store.getAll('applications');
  const orgLicenses = allLicenses.filter(l => providers.some(p => p.id === l.providerId));
  const orgApps = allApps.filter(a => a.orgId === orgId || providers.some(p => p.id === a.providerId));
  const licensedStates = [...new Set(orgLicenses.map(l => l.state))];
  const estRevenue = orgApps.filter(a => a.status === 'approved').reduce((s, a) => s + (a.estMonthlyRevenue || 0), 0);
  ...
  if (pageActions) pageActions.innerHTML = `
    <button class="btn btn-sm" onclick="window.app.navigateTo('organizations')">← Back</button>
  ...
  body.innerHTML = `
    ...
    <div class="card" style="border-top:3px solid var(--brand-600);margin-bottom:20px;">
      <div class="card-body">
        ...
        <div style="display:flex;gap:24px;flex-wrap:wrap;margin-top:8px;font-size:13px;color:var(--gray-600);">
          ...
          <div>Tax ID: <strong>${o.taxId || '—'}</strong></div>
          <div>Taxonomy: <strong>${o.taxonomy || '—'}</strong></div>
        </div>
        <div style="display:flex;gap:24px;flex-wrap:wrap;margin-top:6px;font-size:13px;color:var(--gray-600);">
          ${o.phone ? `<div>Phone: ${escHtml(o.phone)}</div>` : ''}
          ${o.email ? `<div>Email: <a href="mailto:${escAttr(o.email)}">${escHtml(o.email)}</a></div>` : ''}
          ${o.address ? `<div>${escHtml(o.address.street || '')}, ${escHtml(o.address.city || '')}, ${escHtml(o.address.state || '')} ${escHtml(o.address.zip || '')}</div>` : ''}
        </div>
      </div>
    </div>
    ...
    <div class="stats-grid" style="grid-template-columns:repeat(auto-fit,minmax(140px,1fr));margin-bottom:20px;">
      <div class="stat-card"><div class="label">Providers</div><div class="value">${providers.length}</div></div>
      <div class="stat-card"><div class="label">Licenses</div><div class="value blue">${orgLicenses.length}</div></div>
      <div class="stat-card"><div class="label">Licensed States</div><div class="value">${licensedStates.length}</div></div>
    ...
    <div class="card" style="margin-bottom:20px;">
      <div class="card-header"><h3>Providers (${providers.length})</h3></div>
      <div class="card-body" style="padding:0;">
        ${providers.length === 0 ? '<div class="empty-state" style="padding:30px;"><p>No providers in this organization.</p></div>' : `
          <table>
            ...
            <tbody>
              ${providers.map(p => {
                const pLic = allLicenses.filter(l => l.providerId === p.id);
                const pApps = allApps.filter(a => a.providerId === p.id);
                return `
                  <tr style="cursor:pointer;" onclick="window.app.viewProvider('${p.id}')">
                    <td><strong>${escHtml(p.firstName)} ${escHtml(p.lastName)}</strong>${p.credentials ? '<br><span class="text-sm text-muted">' + escHtml(p.credentials) + '</span>' : ''}</td>
                    <td><code style="color:var(--brand-700);">${p.npi || '—'}</code></td>
                    <td class="text-sm">${escHtml(p.specialty) || '—'}</td>
    ...
    <div class="tabs" style="margin-bottom:16px;">
      ...
      <button class="tab" onclick="window.app.orgDetailTab(this, 'od-contacts')">Contacts (${store.getAll('org_contacts').filter(c => c.orgId === orgId).length})</button>
    </div>
            ...
            <table>
              ...
              <tbody>
                ${orgApps.sort((a, b) => (b.createdAt || '').localeCompare(a.createdAt || '')).slice(0, 50).map(a => {
                  const prov = store.getById('providers', a.providerId);
                  return `
                    <tr>
                      <td class="text-sm">${prov ? escHtml(prov.firstName + ' ' + prov.lastName) : '—'}</td>
                      <td><strong>${escHtml(a.payerName || '')}</strong></td>
                      <td>${a.state || '—'}</td>
                      <td><span class="badge badge-${a.status}">${(a.status || '').replace(/_/g, ' ')}</span></td>
                      <td>${a.wave ? `<span class="wave-badge wave-${a.wave}">W${a.wave}</span>` : '—'}</td>
    ...
    <div id="od-contacts" class="hidden">
      ${renderOrgContactsTab(orgId)}
    </div>
  `;
}

function renderOrgContactsTab(orgId) {
  const contacts = store.getAll('org_contacts').filter(c => c.orgId === orgId);
  ...
  return `
    ...
    ${contacts.length === 0 ? '<div class="empty-state" style="padding:30px;"><p>No contacts. Click "+ Add Contact" to add one.</p></div>' : `
      <div class="table-wrap">
        <table>
...
function openOrgContactForm(orgId, contactId) {
  const existing = contactId ? store.getById('org_contacts', contactId) : null;
  const modal = document.getElementById('log-modal');
...
function saveOrgContact() {
  ...
  if (id) {
    store.update('org_contacts', id, data);
    showToast('Contact updated');
  } else {
    const result = store.add('org_contacts', 'org_contact', data);
    if (!result.success) { showToast('Error: ' + result.errors.join(', ')); return; }
...
function openProviderModal(id) {
  ...
  const existing = id ? store.getById('providers', id) : null;
  ...
  form.innerHTML = `
    ...
    <div style="display:flex;gap:8px;align-items:flex-end;margin-bottom:20px;padding:14px 16px;background:var(--gray-50);border:1px solid var(--gray-200);border-radius:var(--radius-lg);">
      <div style="flex:1;">
        ...
        <input type="text" class="form-control" id="prov-npi-lookup" placeholder="Enter 10-digit NPI to auto-fill" value="${escAttr(existing?.npi || '')}" style="font-size:15px;letter-spacing:0.5px;" onkeydown="if(event.key==='Enter'){event.preventDefault();window.app.lookupProviderNPI();}">
      </div>
...
window.saveProvider = function() {
  ...
  if (id) {
    store.update('providers', id, data);
    showToast('Provider updated');
  } else {
    store.add('providers', 'provider', data);
...
function renderSettings() {
  ...
  body.innerHTML = `
    ...
    <div id="settings-import">
      <div class="card">
        <div class="card-header"><h3>Import from V1 Tracker (Google Sheets)</h3></div>
        <div class="card-body">
          <p class="text-sm text-muted mb-4">Fetch credentialing applications directly from the V1 tracker's Google Sheet and import into V2.</p>
          <div style="display:flex;gap:10px;">
            <button class="btn" onclick="window.app.previewV1CloudImport()">Preview Import</button>
            <button class="btn btn-primary" onclick="window.app.runV1CloudImport()">Import from V1</button>
          </div>
      ...
      <div class="card">
        ...
        <div class="card-body">
          <p class="text-sm text-muted mb-4">Paste CSV data exported from the v1 tracker.</p>
          <textarea class="form-control" id="csv-input" rows="6" placeholder="Paste CSV content here..."></textarea>
          <button class="btn btn-primary mt-4" onclick="window.app.runCSVImport()">Import CSV</button>
        </div>
      </div>

      <div class="card">
        <div class="card-header"><h3>Export V2 Data</h3></div>
        <div class="card-body">
          <p class="text-sm text-muted mb-4">Export all v2 data as JSON for backup.</p>
          <div style="display:flex;gap:10px;">
        ...
        <div class="card-body">
          ...
          <div class="form-row">
            <div class="form-group">
              ...
              <input class="form-control" id="caqh-org-id" placeholder="Your CAQH Org ID" value="${escAttr(getCaqhConfig().orgId)}">
            </div>
            <div class="form-group">
              <label>Environment</label>
              <select class="form-control" id="caqh-environment">
                <option value="production" ${getCaqhConfig().environment === 'production' ? 'selected' : ''}>Production</option>
                <option value="sandbox" ${getCaqhConfig().environment === 'sandbox' ? 'selected' : ''}>Sandbox</option>
              </select>
            </div>
          </div>
          <div class="form-row">
            <div class="form-group">
              ...
              <input class="form-control" id="caqh-username" placeholder="CAQH API username" value="${escAttr(getCaqhConfig().username)}">
            </div>
            <div class="form-group">
              <label>API Password</label>
              <input type="password" class="form-control" id="caqh-password" placeholder="CAQH API password" value="${escAttr(getCaqhConfig().password)}">
            </div>
          </div>
...
function openDocChecklist(appId) {
    ...
    <div id="task-page-add-form" style="display:none;margin-bottom:16px;padding:16px;border:1px solid var(--border);border-radius:8px;background:white;box-shadow:var(--shadow);">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:10px;">
        ...
        <select id="task-page-link-app" class="form-control">
          ...
          ${store.getAll('applications').map(a => `<option value="${a.id}">${getStateName(a.state)} — ${a.payerName || getPayerById(a.payerId)?.name || 'Unknown'}</option>`).join('')}
        </select>
...
function renderTaskPageRow(task, today) {
  ...
  const linkedApp = task.linkedAppId ? store.getById('applications', task.linkedAppId) : null;
    ...
    <div id="task-add-form" style="display:none;margin-bottom:16px;padding:12px;border:1px solid var(--border);border-radius:8px;background:var(--gray-50);">
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:8px;">
        ...
        <select id="task-link-app" class="form-control">
          <option value="">Link to application (optional)</option>
          ${store.getAll('applications').map(a => `<option value="${a.id}">${getStateName(a.state)} — ${a.payerName}</option>`).join('')}
        </select>
...
function renderDocChecklistTool() {
...
function renderFeeScheduleTool() {
  const CPT_CODES = [
...
function renderPayerPortalTool() {
...
function renderExpirationAlertsTool() {
  const licenses = store.getAll('licenses');
  const apps = store.getAll('applications');
  const now = new Date();
  const alerts = [];
...
function renderStatusExportTool() {
  const body = document.getElementById('page-body');
  body.innerHTML = `
    <div class="card">
...
function renderStateLookupTool() {
  const policies = store.getAll('telehealth_policies').length > 0
    ? store.getAll('telehealth_policies')
    : TELEHEALTH_POLICIES;
  const body = document.getElementById('page-body');
  body.innerHTML = `
    <div class="card">
...
function renderDeadlineTimelineTool() {
  const apps = store.getAll('applications');
  const licenses = store.getAll('licenses');
  const tasks = store.getAll('tasks').filter(t => !t.completed && t.dueDate);
  const now = new Date();
  const items = [];
...
function renderLetterGeneratorTool() {
        ...
        <div class="card-body" style="padding:0;">
          <div class="table-wrap" style="box-shadow:none;border:none;">
            <table>
              ...
              <tbody>
                ${TAXONOMY_CODES.slice(0, 20).map(t => `
...
function openOrgModal(id) {
  ...
  document.getElementById('log-modal-body').innerHTML = `
    ...
    <div class="form-row">
      ...
      <div class="form-group"><label>Tax ID</label><input type="text" class="form-control" id="org-taxid" value="${escAttr(existing?.taxId || '')}" placeholder="XX-XXXXXXX"></div>
    </div>
    <div class="form-row">
    ...
    <div class="form-group"><label>Taxonomy Code</label><input type="text" class="form-control" id="org-taxonomy" value="${escAttr(existing?.taxonomy || '')}" placeholder="e.g. 2084P0800X"></div>
    <div class="form-group"><label>Street Address</label><input type="text" class="form-control" id="org-street" value="${escAttr(existing?.address?.street || '')}" placeholder="123 Main St, Suite 100"></div>
    <div class="form-row">
      <div class="form-group"><label>City</label><input type="text" class="form-control" id="org-city" value="${escAttr(existing?.address?.city || '')}"></div>
      <div class="form-group" style="flex:0 0 80px;"><label>State</label><input type="text" class="form-control" id="org-state" value="${escAttr(existing?.address?.state || '')}" maxlength="2" placeholder="FL"></div>
      <div class="form-group" style="flex:0 0 100px;"><label>ZIP</label><input type="text" class="form-control" id="org-zip" value="${escAttr(existing?.address?.zip || '')}" placeholder="12345"></div>
    </div>
    <div style="margin-top:16px;display:flex;gap:10px;justify-content:flex-end;">
      <button class="btn" onclick="window.closeLogModal()">Cancel</button>
      ${existing ? `<button class="btn" style="color:var(--red);" onclick="window.app.deleteOrg('${id}')">Delete</button>` : ''}
...
function renderUsersPage() {
  ...
  body.innerHTML = `
      ...
      <div class="card-body" style="padding:0;">
        ${users.length === 0 ? `
          <div class="empty-state" style="padding:40px;">
            <h3>No Users Yet</h3>
            <p>Click "+ Create User" to add your first platform user.</p>
          </div>
        ` : `
          <div class="table-wrap" style="box-shadow:none;border:none;">
            <table>
                ...
                ${users.map(u => {
                  ...
                  return `
                    <tr>
                      ...
                      <td><span class="badge badge-${u.role === 'superadmin' ? 'approved' : u.role === 'admin' ? 'submitted' : 'active'}">${escHtml(u.role || 'staff')}</span></td>
                      <td>${org ? escHtml(org.name) : '<span class="text-muted">—</span>'}</td>
                      <td><span class="badge badge-${u.active !== false ? 'active' : 'expired'}">${u.active !== false ? 'Active' : 'Inactive'}</span></td>
                      <td class="text-sm text-muted">${u.createdAt ? new Date(u.createdAt).toLocaleDateString() : '—'}</td>
                      <td>
                        <button class="btn btn-sm" onclick="window.app.editUser('${u.id}')">Edit</button>
                        <button class="btn btn-sm" onclick="window.app.resetUserPassword('${u.id}')" title="Generate new password">Reset PW</button>
                        <button class="btn btn-sm" style="color:var(--red);" onclick="window.app.deleteUser('${u.id}')">Delete</button>
                      </td>
                    </tr>`;
                }).join('')}
              </tbody>
    ...
    <div class="card" style="margin-top:20px;">
      <div class="card-header"><h3>Password Generator</h3></div>
      <div class="card-body">
        <p class="text-sm text-muted" style="margin-bottom:12px;">Generate secure passwords for user accounts.</p>
        <div style="display:flex;gap:10px;align-items:flex-end;">
          <div style="flex:0 0 120px;">
            <label style="display:block;font-size:12px;font-weight:600;color:var(--gray-600);margin-bottom:4px;">Length</label>
            <input type="number" class="form-control" id="pw-length" value="16" min="8" max="64">
          </div>
          <button class="btn btn-primary" onclick="window.app.generateNewPassword()" style="height:40px;">Generate Password</button>
        </div>
        <div id="pw-result" style="display:none;margin-top:16px;padding:14px 16px;background:var(--gray-50);border:1px solid var(--gray-200);border-radius:var(--radius-lg);">
          <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;">
            <code id="pw-output" style="font-size:16px;font-weight:700;letter-spacing:1px;color:var(--brand-700);word-break:break-all;"></code>
            <button class="btn btn-sm btn-primary" onclick="navigator.clipboard.writeText(document.getElementById('pw-output').textContent);window.app.showToast('Password copied!')">Copy</button>
          </div>
          <div class="text-sm text-muted" style="margin-top:6px;">Contains uppercase, lowercase, numbers, and symbols.</div>
        </div>
      </div>
    </div>
  `;
...
function openCreateUserModal(editId) {
  const existing = editId ? (store.getAll('users') || []).find(u => u.id === editId) : null;
  const orgs = store.getAll('organizations');
  const pw = existing ? '' : generatePassword(16);

  const modal = document.getElementById('log-modal');
  document.getElementById('log-modal-title').textContent = existing ? 'Edit User' : 'Create User';
  document.getElementById('log-modal-body').innerHTML = `
    <input type="hidden" id="edit-user-id" value="${editId || ''}">
    <div class="form-row">
      <div class="form-group"><label>First Name *</label><input type="text" class="form-control" id="user-first" value="${escAttr(existing?.firstName || '')}" placeholder="First name"></div>
      <div class="form-group"><label>Last Name *</label><input type="text" class="form-control" id="user-last" value="${escAttr(existing?.lastName || '')}" placeholder="Last name"></div>
    </div>
    <div class="form-row">
      <div class="form-group"><label>Email *</label><input type="email" class="form-control" id="user-email" value="${escAttr(existing?.email || '')}" placeholder="user@example.com"></div>
      <div class="form-group">
        <label>Role *</label>
        <select class="form-control" id="user-role">
          <option value="staff" ${(!existing || existing.role === 'staff') ? 'selected' : ''}>Staff</option>
          <option value="admin" ${existing?.role === 'admin' ? 'selected' : ''}>Admin</option>
          <option value="owner" ${existing?.role === 'owner' ? 'selected' : ''}>Owner</option>
          <option value="readonly" ${existing?.role === 'readonly' ? 'selected' : ''}>Read Only</option>
          <option value="provider" ${existing?.role === 'provider' ? 'selected' : ''}>Provider</option>
          <option value="superadmin" ${existing?.role === 'superadmin' ? 'selected' : ''}>SuperAdmin</option>
        </select>
      </div>
    </div>
    <div class="form-row">
    ...
    ${!existing ? `
      <div class="form-group">
        ...
        <div style="display:flex;gap:8px;align-items:center;">
          <input type="text" class="form-control" id="user-password" value="${escAttr(pw)}" style="font-family:monospace;font-size:14px;letter-spacing:0.5px;" readonly>
          <button class="btn btn-sm" onclick="document.getElementById('user-password').value=window.app._genPw();window.app.showToast('New password generated');">Regenerate</button>
          <button class="btn btn-sm btn-primary" onclick="navigator.clipboard.writeText(document.getElementById('user-password').value);window.app.showToast('Password copied!')">Copy</button>
        </div>
        <div class="text-sm text-muted" style="margin-top:4px;">Copy this password and share it securely with the user.</div>
      </div>
    ` : ''}
    <div class="form-group">
      <label>Phone</label>
      <input type="text" class="form-control" id="user-phone" value="${escAttr(existing?.phone || '')}" placeholder="(555) 123-4567">
    </div>
    <div style="margin-top:16px;display:flex;gap:10px;justify-content:flex-end;">
      <button class="btn" onclick="window.closeLogModal()">Cancel</button>
      <button class="btn btn-primary" onclick="window.app.saveUser()">Save User</button>
    </div>
  `;
  modal.classList.add('active');
}
...
function renderSendEmailPage() {
  ...
  body.innerHTML = `
      ...
      <div class="card-body">
        <div class="form-row">
          ...
          <div class="form-group">
            ...
            <select class="form-control" id="email-recipient-select" onchange="window.app.onEmailRecipientSelect()">
              <option value="">— Select recipient —</option>
              <optgroup label="Providers">
                ${providers.filter(p => p.email).map(p => `<option value="${escAttr(p.email)}" data-name="${escAttr(p.firstName + ' ' + p.lastName)}">${escHtml(p.firstName)} ${escHtml(p.lastName)} — ${escHtml(p.email)}</option>`).join('')}
              </optgroup>
              <optgroup label="Users">
                ${users.filter(u => u.email).map(u => `<option value="${escAttr(u.email)}" data-name="${escAttr(u.firstName + ' ' + u.lastName)}">${escHtml(u.firstName || '')} ${escHtml(u.lastName || '')} — ${escHtml(u.email)}</option>`).join('')}
              </optgroup>
            </select>
          </div>
        </div>
        <div class="form-row">
          <div class="form-group" style="flex:2;">
            <label>CC</label>
            <input type="text" class="form-control" id="email-cc" placeholder="cc@example.com (comma-separated)">
          </div>
          <div class="form-group">
            <label>Template</label>
            <select class="form-control" id="email-send-template" onchange="window.app.onEmailTemplateSelect()">
              <option value="">— No template —</option>
              ${templates.map(t => `<option value="${t.id}">${escHtml(t.name)}</option>`).join('')}
              <option value="_welcome">Welcome / Account Created</option>
              <option value="_password_reset">Password Reset</option>
              <option value="_invoice">Invoice Notification</option>
            </select>
          </div>
        </div>
        <div class="form-group">
          <label>Subject *</label>
          <input type="text" class="form-control" id="email-subject" placeholder="Email subject line">
        </div>
        <div class="form-group">
          <label>Body *</label>
          <textarea class="form-control" id="email-body" rows="12" placeholder="Write your email message here..." style="font-size:14px;line-height:1.6;"></textarea>
        </div>
        <div class="form-row" style="margin-top:4px;">
          <div class="form-group">
            <label>Related Application</label>
            <select class="form-control" id="email-send-app">
              <option value="">— None —</option>
              ${apps.map(a => `<option value="${a.id}">${escHtml(a.payerName || '')} — ${escHtml(a.state || '')}</option>`).join('')}
            </select>
          </div>
        </div>
        <div style="display:flex;gap:10px;margin-top:16px;justify-content:space-between;align-items:center;">
          <div class="text-sm text-muted">Emails open in your default email client via mailto.</div>
          <div style="display:flex;gap:10px;">
            <button class="btn" onclick="window.app.copyEmailToClipboard()">Copy to Clipboard</button>
            <button class="btn btn-primary" onclick="window.app.sendEmailNow()">
              <svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M1.5 8l5 3 8-7"/><path d="M6.5 11v3.5l2.5-2"/></svg>
              Send Email
            </button>
          </div>
        </div>
    ...
    <div class="card" style="margin-top:20px;">
      <div class="card-header"><h3>Sent Email Log</h3></div>
      <div class="card-body" style="padding:0;">
        ${(() => {
          const log = store.getAll('emailLog') || [];
          if (log.length === 0) return '<div class="empty-state" style="padding:30px;"><p class="text-muted">No emails sent yet.</p></div>';
          return '<div class="table-wrap" style="box-shadow:none;border:none;"><table><thead><tr><th>Date</th><th>To</th><th>Subject</th><th>Status</th></tr></thead><tbody>' +
            log.sort((a, b) => (b.sentAt || '').localeCompare(a.sentAt || '')).slice(0, 50).map(e => `
              <tr>
                <td class="text-sm">${e.sentAt ? new Date(e.sentAt).toLocaleString() : '—'}</td>
                <td>${escHtml(e.to || '')}</td>
                <td class="truncate" style="max-width:300px;">${escHtml(e.subject || '')}</td>
                <td><span class="badge badge-${e.status === 'sent' ? 'active' : 'pending'}">${escHtml(e.status || 'sent')}</span></td>
              </tr>`).join('') +
            '</tbody></table></div>';
        })()}
      </div>
...
function renderInvoicesPage() {
  ...
  const totalPending = invoices.filter(i => i.status !== 'paid' && i.status !== 'void').reduce((s, i) => s + (parseFloat(i.total) || 0), 0);

  body.innerHTML = `
    <div class="stats-grid" style="margin-bottom:20px;">
      <div class="stat-card"><div class="label">Total Invoices</div><div class="value">${invoices.length}</div></div>
      <div class="stat-card"><div class="label">Paid</div><div class="value green">$${totalPaid.toLocaleString('en-US', { minimumFractionDigits: 2 })}</div></div>
      <div class="stat-card"><div class="label">Outstanding</div><div class="value" style="color:var(--gold);">$${totalPending.toLocaleString('en-US', { minimumFractionDigits: 2 })}</div></div>
      <div class="stat-card"><div class="label">Overdue</div><div class="value" style="color:var(--red);">${invoices.filter(i => i.status === 'overdue').length}</div></div>
    </div>
      ...
      <div class="card-body" style="padding:0;">
        ${invoices.length === 0 ? `
          <div class="empty-state" style="padding:40px;">
            ...
            <p>Click "+ Create Invoice" to generate your first invoice.</p>
          </div>
        ` : `
          <div class="table-wrap" style="box-shadow:none;border:none;">
            <table>
              ...
              <tbody>
                ${invoices.sort((a, b) => (b.createdAt || '').localeCompare(a.createdAt || '')).map(inv => `
                  <tr>
                    ...
                    <td class="truncate" style="max-width:200px;">${escHtml(inv.description || '')}</td>
                    <td><strong>$${(parseFloat(inv.total) || 0).toLocaleString('en-US', { minimumFractionDigits: 2 })}</strong></td>
                    <td class="text-sm">${inv.invoiceDate ? new Date(inv.invoiceDate).toLocaleDateString() : '—'}</td>
                    <td class="text-sm">${inv.dueDate ? new Date(inv.dueDate).toLocaleDateString() : '—'}</td>
                    <td><span class="badge badge-${inv.status === 'paid' ? 'approved' : inv.status === 'overdue' ? 'denied' : inv.status === 'void' ? 'expired' : 'submitted'}">${escHtml(inv.status || 'draft')}</span></td>
                    <td>
                      <button class="btn btn-sm" onclick="window.app.viewInvoice('${inv.id}')">View</button>
                      <button class="btn btn-sm" onclick="window.app.editInvoice('${inv.id}')">Edit</button>
                      ${inv.status !== 'paid' ? `<button class="btn btn-sm" style="color:var(--green);" onclick="window.app.markInvoicePaid('${inv.id}')">Mark Paid</button>` : ''}
                    </td>
                  </tr>
                `).join('')}
              </tbody>
...
function openCreateInvoiceModal(editId) {
  const existing = editId ? (store.getAll('invoices') || []).find(i => i.id === editId) : null;
  const orgs = store.getAll('organizations');
  const providers = store.getAll('providers');
  const nextNum = existing ? existing.invoiceNumber : 'INV-' + String((store.getAll('invoices') || []).length + 1001).padStart(4, '0');
  const today = new Date().toISOString().split('T')[0];
  const dueDate = existing?.dueDate || new Date(Date.now() + 30 * 86400000).toISOString().split('T')[0];

  const modal = document.getElementById('log-modal');
  document.getElementById('log-modal-title').textContent = existing ? 'Edit Invoice' : 'Create Invoice';
  document.getElementById('log-modal-body').innerHTML = `
    <input type="hidden" id="edit-invoice-id" value="${editId || ''}">
    <div class="form-row">
      <div class="form-group"><label>Invoice #</label><input type="text" class="form-control" id="inv-number" value="${escAttr(nextNum)}" ${existing ? '' : 'readonly'}></div>
      <div class="form-group">
        <label>Status</label>
        <select class="form-control" id="inv-status">
          <option value="draft" ${(!existing || existing.status === 'draft') ? 'selected' : ''}>Draft</option>
          <option value="sent" ${existing?.status === 'sent' ? 'selected' : ''}>Sent</option>
    ...
    <div class="form-row">
      <div class="form-group"><label>Client / Organization *</label><input type="text" class="form-control" id="inv-client" value="${escAttr(existing?.clientName || (orgs[0]?.name || ''))}" placeholder="Client name"></div>
      <div class="form-group"><label>Client Email</label><input type="email" class="form-control" id="inv-client-email" value="${escAttr(existing?.clientEmail || '')}" placeholder="billing@client.com"></div>
    </div>
    <div class="form-row">
      <div class="form-group"><label>Invoice Date</label><input type="date" class="form-control" id="inv-date" value="${escAttr(existing?.invoiceDate || today)}"></div>
      <div class="form-group"><label>Due Date</label><input type="date" class="form-control" id="inv-due" value="${escAttr(dueDate)}"></div>
    </div>
    <div class="form-group">
      <label>Description *</label>
      <textarea class="form-control" id="inv-desc" rows="3" placeholder="Credentialing services for...">${existing?.description || ''}</textarea>
    </div>

    <div id="inv-line-items">
      <label style="display:block;font-size:12px;font-weight:600;color:var(--gray-600);text-transform:uppercase;letter-spacing:0.4px;margin-bottom:8px;">Line Items</label>
      <div id="inv-items-list">
        ${(existing?.items || [{ description: 'Credentialing Application Processing', qty: 1, rate: 250 }]).map((item, idx) => `
          <div class="form-row inv-item-row" style="align-items:flex-end;margin-bottom:8px;">
            <div class="form-group" style="flex:3;"><input type="text" class="form-control" placeholder="Item description" value="${escAttr(item.description || '')}" data-inv-field="desc" data-inv-idx="${idx}"></div>
            <div class="form-group" style="flex:0 0 70px;"><input type="number" class="form-control" placeholder="Qty" value="${item.qty || 1}" min="1" data-inv-field="qty" data-inv-idx="${idx}" onchange="window.app.recalcInvoice()"></div>
            <div class="form-group" style="flex:0 0 100px;"><input type="number" class="form-control" placeholder="Rate" value="${item.rate || 0}" step="0.01" data-inv-field="rate" data-inv-idx="${idx}" onchange="window.app.recalcInvoice()"></div>
            <div class="form-group" style="flex:0 0 100px;"><input type="text" class="form-control" value="$${((item.qty || 1) * (item.rate || 0)).toFixed(2)}" readonly data-inv-field="subtotal" data-inv-idx="${idx}" style="background:var(--gray-50);font-weight:600;"></div>
            <button class="btn btn-sm" onclick="this.closest('.inv-item-row').remove();window.app.recalcInvoice();" style="color:var(--red);height:40px;">&times;</button>
          </div>
        `).join('')}
      </div>
      <button class="btn btn-sm" onclick="window.app.addInvoiceItem()" style="margin-top:4px;">+ Add Line Item</button>
    </div>

    <div style="text-align:right;margin-top:16px;padding:12px;background:var(--gray-50);border-radius:var(--radius-lg);">
      <div style="font-size:13px;color:var(--gray-600);">Subtotal: <strong id="inv-subtotal">$0.00</strong></div>
      <div style="font-size:18px;font-weight:800;color:var(--gray-900);margin-top:4px;">Total: <span id="inv-total">$0.00</span></div>
    </div>

    <div class="form-group" style="margin-top:12px;">
      <label>Notes</label>
      <textarea class="form-control" id="inv-notes" rows="2" placeholder="Payment terms, notes...">${existing?.notes || 'Payment due within 30 days. Thank you for your business.'}</textarea>
    </div>
  ...
  // Initial calc
  setTimeout(() => { window.app.recalcInvoice(); }, 50);
}

// ─── CAQH ProView Manager ───

function renderCaqhManager() {
  const body = document.getElementById('page-body');
  const providers = store.getAll('providers');
  const configured = isCaqhConfigured();
...
function renderCaqhAttestationTab(providers, tracking) {
  const now = new Date();
...
window.app = {
  ...
  // Document Checklist
  openDocChecklist(appId) { openDocChecklist(appId); },
  toggleDoc(appId, docId) {
    const app = store.getById('applications', appId);
    if (!app) return;
    const docs = app.documentChecklist || {};
    ...
    store.update('applications', appId, { documentChecklist: docs });
    openDocChecklist(appId);
  },
  ...
  uncheckAllDocs(appId) {
    store.update('applications', appId, { documentChecklist: {} });
    openDocChecklist(appId);
  },
  ...
  savePageTask() {
    ...
    const result = store.add('tasks', 'task', {
      title,
  ...
  refreshTasksPage() { renderTasksPage(); },
  ...
  async deleteProvider(id) {
    ...
    store.remove('providers', id);
    renderProviders();
    showToast('Provider deleted');
  },
  provDetailTab(btn, tabId) {
    btn.closest('.tabs').querySelectorAll('.tab').forEach(t => t.classList.remove('active'));
    btn.classList.add('active');
    ['pd-licenses', 'pd-applications', 'pd-activity', 'pd-education', 'pd-boards', 'pd-work-history', 'pd-malpractice', 'pd-cme', 'pd-references', 'pd-documents'].forEach(id => {
      const el = document.getElementById(id);
      if (el) el.classList.toggle('hidden', id !== tabId);
    });
  },

  // Provider Profile Tabs (Education, Boards, etc.)
  openProvProfileForm(providerId, collection, label, existingId) { openProvProfileForm(providerId, collection, label, existingId); },
  editProvProfileRecord(providerId, collection, label, recordId) { openProvProfileForm(providerId, collection, label, recordId); },
  async deleteProvProfileRecord(providerId, collection, recordId) {
    if (!await appConfirm('Delete this record?', { title: 'Delete Record', okLabel: 'Delete', okClass: 'btn-danger' })) return;
    deleteProvProfileRecord(providerId, collection, recordId);
  },
  saveProvProfileRecord() { saveProvProfileRecord(); },

  // Exclusion Screening
  async screenAllProviders() {
    const providers = store.getAll('providers');
    if (providers.length === 0) { showToast('No providers to screen'); return; }
    const today = new Date().toISOString().split('T')[0];
    for (const p of providers) {
      store.add('exclusion_checks', 'exclusion_check', {
        providerId: p.id,
        checkDate: today,
        result: 'clear',
        source: 'OIG/SAM Manual Check',
        notes: 'Batch screening — no exclusions found',
      });
    }
    showToast(`Screened ${providers.length} providers`);
    renderExclusionScreening();
  },
  screenProvider(providerId) {
    const today = new Date().toISOString().split('T')[0];
    store.add('exclusion_checks', 'exclusion_check', {
      providerId,
      checkDate: today,
      result: 'clear',
      source: 'OIG/SAM Manual Check',
      notes: 'Individual screening — no exclusions found',
    });
    showToast('Provider screened — Clear');
    renderExclusionScreening();
  },
  manualExclusionResult(providerId) {
    const modal = document.getElementById('log-modal');
    const provider = store.getById('providers', providerId);
    document.getElementById('log-modal-title').textContent = `Manual Exclusion Entry — ${provider?.firstName || ''} ${provider?.lastName || ''}`;
    document.getElementById('log-modal-body').innerHTML = `
      <input type="hidden" id="exc-provider" value="${providerId}">
      <div class="form-group"><label>Check Date</label><input type="date" class="form-control" id="exc-date" value="${new Date().toISOString().split('T')[0]}"></div>
      <div class="form-group"><label>Result</label><select class="form-control" id="exc-result">
        <option value="clear">Clear</option>
        <option value="excluded">Excluded</option>
        <option value="error">Error</option>
      </select></div>
      <div class="form-group"><label>Source</label><input type="text" class="form-control" id="exc-source" value="OIG/SAM" placeholder="e.g. OIG LEIE, SAM.gov"></div>
      <div class="form-group"><label>Notes</label><input type="text" class="form-control" id="exc-notes" placeholder="Details..."></div>
      <div style="margin-top:16px;display:flex;gap:10px;justify-content:flex-end;">
        <button class="btn" onclick="window.closeLogModal()">Cancel</button>
        <button class="btn btn-primary" onclick="window.app.saveExclusionResult()">Save Result</button>
      </div>
    `;
    modal.classList.add('active');
  },
  saveExclusionResult() {
    const providerId = document.getElementById('exc-provider')?.value;
    store.add('exclusion_checks', 'exclusion_check', {
      providerId,
      checkDate: document.getElementById('exc-date')?.value || new Date().toISOString().split('T')[0],
      result: document.getElementById('exc-result')?.value || 'clear',
      source: document.getElementById('exc-source')?.value?.trim() || '',
      notes: document.getElementById('exc-notes')?.value?.trim() || '',
    });
    window.closeLogModal();
    showToast('Exclusion result saved');
    renderExclusionScreening();
  },
  ...
  async deleteOrgContact(orgId, contactId) {
    ...
    store.remove('org_contacts', contactId);
    showToast('Contact deleted');
    renderOrgDetail(orgId);
  },
  saveOrgContact() { saveOrgContact(); },
  ...
  saveOrg() {
    ...
    const data = {
      ...
      address: {
        street: document.getElementById('org-street')?.value?.trim() || '',
        city: document.getElementById('org-city')?.value?.trim() || '',
        state: document.getElementById('org-state')?.value?.trim().toUpperCase() || '',
        zip: document.getElementById('org-zip')?.value?.trim() || '',
      },
    };
    if (!data.name) { showToast('Organization name is required'); return; }
    if (id) {
      store.update('organizations', id, data);
      showToast('Organization updated');
    } else {
      data.createdAt = new Date().toISOString();
      store.add('organizations', 'organization', data);
  ...
  async lookupProviderNPI() {
    ...
    if (!npiInput || !resultDiv) { console.warn('NPI lookup: DOM elements not found'); return; }
    ...
    if (btn) { btn.disabled = true; btn.innerHTML = '<div class="spinner" style="width:16px;height:16px;margin:0;border-width:2px;"></div>'; }
    resultDiv.style.display = 'block';
    resultDiv.innerHTML = '<div style="text-align:center;padding:8px;color:var(--gray-500);font-size:13px;">Looking up NPI...</div>';

    try {
      const prov = await lookupNPI(npi);
      if (!prov) {
      ...
      resultDiv.innerHTML = '<div class="alert alert-danger" style="margin:0;">Lookup failed: ' + escHtml(err.message) + '. Check browser console for details.</div>';
    } finally {
      if (btn) { btn.disabled = false; btn.innerHTML = '<svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2"><circle cx="7" cy="7" r="5"/><path d="M11 11l3.5 3.5"/></svg> Lookup'; }
    }
  },
  ...
  async runTaxonomySearch() {
    ...
    if (type === 'codes') {
      ...
      const results = searchTaxonomyCodes(query);
      ...
      try {
        const prov = await lookupNPI(query.trim());
      ...
      try {
        ...
        const results = await searchProviders(opts);
      ...
      try {
        const opts = { limit: 30 };
        if (state) opts.state = state;
        const results = await searchByTaxonomy(query, opts);
  ...
  async completeFollowupPrompt(fuId) {
    ...
    completeFollowup(fuId, outcome, nextAction || '');
  ...
  previewBatch() {
    ...
    const result = generateBatch({ strategyId, targetStates, excludeExisting });
    ...
    const summary = summarizeBatch(result.batch);
  ...
  async commitCurrentBatch() {
    ...
    const result = commitBatch(window._pendingBatch);
  ...
  generateAppEmail() {
    ...
    const result = generateEmailForApplication(appId, templateId);
  ...
  generateExpansionEmailBatch() {
    ...
    const result = generateExpansionEmails(states);
    const output = document.getElementById('email-output');
  ...
  generateEscalationEmail(appId) {
    const result = generateEmailForApplication(appId, 'escalation');
  ...
  async previewV1CloudImport() {
    const el = document.getElementById('import-results');
    el.innerHTML = '<div class="alert alert-info">Fetching data from V1 Google Sheets...</div>';
    const result = await previewV1CloudImport();
    if (!result.success) {
    ...
    el.innerHTML = `
      <div class="alert alert-info">
        <strong>Preview:</strong> ${result.total} records found. ${result.imported} would be imported, ${result.skipped} would be skipped (already exist).
        ${result.warnings.length > 0 ? '<br>Warnings: ' + result.warnings.slice(0, 3).join('; ') : ''}
        ${result.unmappedPayers.length > 0 ? '<br>Unmapped payers: ' + result.unmappedPayers.join(', ') : ''}
      </div>
    `;
  },

  async runV1CloudImport() {
    const el = document.getElementById('import-results');
    el.innerHTML = '<div class="alert alert-info">Importing from V1 Google Sheets...</div>';
    const result = await importFromV1Cloud();
    if (!result.success) {
      el.innerHTML = `<div class="alert alert-danger">${result.error}</div>`;
      return;
    }
    el.innerHTML = `<div class="alert alert-success">Imported ${result.imported} applications and ${result.followups.length} follow-ups. ${result.skipped} skipped.
      ${result.unmappedPayers.length > 0 ? '<br>Unmapped payers: ' + result.unmappedPayers.join(', ') : ''}
    </div>`;
    showToast(`Imported ${result.imported} applications`);
    navigateTo('applications');
  },

  runCSVImport() {
    const csv = document.getElementById('csv-input').value;
    if (!csv.trim()) { showToast('Paste CSV data first'); return; }
    const result = importFromCSV(csv);
    const el = document.getElementById('import-results') || document.getElementById('page-body');
    if (!result.success) {
  ...
  exportData() {
    const data = exportV2Data();
    const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `ennhealth-cred-v2-backup-${new Date().toISOString().split('T')[0]}.json`;
    a.click();
  ...
  filterPolicyRegion(value) {
    filters._policyRegion = value;
    renderStatePolicies();
  },
  showPolicyDetail(stateCode) {
    const pol = getLivePolicyByState(stateCode);
    if (!pol) return;
    const licenses = store.getAll('licenses').filter(l => l.state === stateCode);
    const apps = store.getAll('applications').filter(a => a.state === stateCode);
    const authColor = pol.practiceAuthority === 'full' ? 'var(--green)' : pol.practiceAuthority === 'reduced' ? 'var(--gold)' : 'var(--red)';
    const scoreColor = pol.readinessScore >= 7 ? 'var(--green)' : pol.readinessScore >= 5 ? 'var(--gold)' : 'var(--red)';
  ...
  printPage() {
    ...
    header.textContent = `EnnHealth Telehealth Strategy — ${document.getElementById('page-title').textContent} — Printed ${new Date().toLocaleDateString()} ${new Date().toLocaleTimeString()}`;
    window.print();
  },
  ...
  bulkUpdateStatus() {
    ...
    selected.forEach(id => store.update('applications', id, { status }));
  ...
  bulkUpdateWave() {
    ...
    const selected = Array.from(document.querySelectorAll('.app-checkbox:checked')).map(el => el.dataset.appId);
    selected.forEach(id => store.update('applications', id, { wave: Number(wave) }));
    showToast(`Updated ${selected.length} applications to Wave ${wave}`);
    renderAppTable();
  },
  exportSelectedCSV() {
    const selected = Array.from(document.querySelectorAll('.app-checkbox:checked')).map(el => el.dataset.appId);
    const apps = selected.map(id => store.getById('applications', id)).filter(Boolean);
  ...
  async bulkDelete() {
    ...
    selected.forEach(id => store.remove('applications', id));
  ...
  generateStatusReport() {
    ...
    if (type === 'executive') {
      ...
      report += `Organization: ${org.name || 'EnnHealth Psychiatry'}\n`;
  ...
  async setCaqhId(providerId) {
    ...
    if (!id) return;
    store.update('providers', providerId, { caqhId: id.trim() });
    showToast('CAQH ID saved');
    renderCaqhManager();
  },

  async checkCaqhStatus(providerId) {
    const prov = store.getById('providers', providerId);
    if (!prov?.caqhId) { showToast('No CAQH ID set for this provider'); return; }
    if (!isCaqhConfigured()) { showToast('CAQH API credentials not configured'); return; }
  ...
  async viewCaqhProfile(providerId) {
    ...
    try {
      const profile = await getProviderProfile(prov.caqhId);
      const modal = document.getElementById('log-modal');
      document.getElementById('log-modal-title').textContent = `CAQH Profile — ${prov.firstName} ${prov.lastName}`;
      document.getElementById('log-modal-body').innerHTML = `
  ...
  async runBatchCaqhCheck() {
    ...
    try {
      const results = await batchStatusCheck();
      results.forEach(r => updateTrackingFromResult(r));
  ...
  saveManualCaqh() {
    ...
    if (caqhId) {
      store.update('providers', providerId, { caqhId });
    }
  ...
  async addProviderToRoster(providerId) {
    const prov = store.getById('providers', providerId);
    if (!prov) return;
    if (!isCaqhConfigured()) { showToast('CAQH API not configured'); return; }
...
// if (body.action === 'nppes_proxy') return sendJson(handleNppesProxy(body));</div>
  ...
  async testCaqhConnection() {
    if (!isCaqhConfigured()) { showToast('Enter credentials first'); return; }
    ...
    try {
      ...
      await getRosterStatus(testProv.caqhId);
  ...
  generateLetter() {
    ...
    const orgName = org.name || 'EnnHealth Psychiatry';
  ...
  openCreateUserModal(id) { openCreateUserModal(id); },
  editUser(id) { openCreateUserModal(id); },
  _genPw() { return generatePassword(parseInt(document.getElementById('pw-length')?.value) || 16); },
  generateNewPassword() {
  ...
  saveUser() {
    ...
    const data = {
      ...
      lastName: document.getElementById('user-last')?.value?.trim(),
      email: document.getElementById('user-email')?.value?.trim(),
      role: document.getElementById('user-role')?.value || 'staff',
      orgId: document.getElementById('user-org')?.value || '',
    ...
    const existing = (store.getAll('users') || []).find(u => u.email === data.email && u.id !== id);
    if (existing) {
    ...
    if (id) {
      ...
      data.password = pw; // Store hashed in production; plaintext for local demo
      data.createdAt = new Date().toISOString();
      store.add('users', 'user', data);
  ...
  async resetUserPassword(id) {
    const user = (store.getAll('users') || []).find(u => u.id === id);
    if (!user) return;
    const pw = generatePassword(16);
    store.update('users', id, { password: pw });

    const modal = document.getElementById('log-modal');
    document.getElementById('log-modal-title').textContent = 'New Password Generated';
    document.getElementById('log-modal-body').innerHTML = `
      <div style="margin-bottom:12px;">
        <p>New password for <strong>${escHtml(user.firstName || '')} ${escHtml(user.lastName || '')}</strong> (${escHtml(user.email || '')}):</p>
      </div>
      <div style="padding:16px;background:var(--gray-50);border:1px solid var(--gray-200);border-radius:var(--radius-lg);display:flex;justify-content:space-between;align-items:center;">
        <code style="font-size:18px;font-weight:700;letter-spacing:1px;color:var(--brand-700);">${escHtml(pw)}</code>
        <button class="btn btn-sm btn-primary" onclick="navigator.clipboard.writeText('${escAttr(pw)}');window.app.showToast('Password copied!')">Copy</button>
      </div>
      <div class="text-sm text-muted" style="margin-top:8px;">Share this password securely with the user. They should change it after first login.</div>
      <div style="margin-top:16px;text-align:right;">
        <button class="btn btn-primary" onclick="window.closeLogModal()">Done</button>
      </div>
    `;
    modal.classList.add('active');
  },
  async deleteUser(id) {
    if (!await appConfirm('Delete this user? This cannot be undone.', { title: 'Delete User', okLabel: 'Delete', okClass: 'btn-danger' })) return;
  ...
  onEmailTemplateSelect() {
    ...
    const builtInTemplates = {
      ...
      _invoice: {
        subject: `Invoice from ${orgName} — [Invoice #]`,
        body: `Dear [Client Name],\n\nPlease find attached your invoice [Invoice #] for credentialing services.\n\nAmount Due: $[amount]\nDue Date: [date]\n\nPayment can be made via [payment method].\n\nIf you have any questions regarding this invoice, please don't hesitate to contact us.\n\nThank you for your business.\n\nBest regards,\n${orgName}`,
      },
    };

    if (templateId && builtInTemplates[templateId]) {
      const t = builtInTemplates[templateId];
      if (subjectEl) subjectEl.value = t.subject;
      if (bodyEl) bodyEl.value = t.body;
    } else if (templateId) {
      // Use email-generator templates
      const appSelect = document.getElementById('email-send-app');
  ...
  sendEmailNow() {
    ...
    if (!to || !subject || !body) {
      showToast('To, Subject, and Body are required');
      return;
    }
    ...
    store.add('emailLog', 'email', {
      ...
      appId: document.getElementById('email-send-app')?.value || '',
    });

    // Open email client
    window.open(mailto, '_blank');
    showToast('Email opened in your email client');
    renderSendEmailPage();
  },
  ...
  openCreateInvoiceModal(id) { openCreateInvoiceModal(id); },
  editInvoice(id) { openCreateInvoiceModal(id); },
  addInvoiceItem() {
    const list = document.getElementById('inv-items-list');
    if (!list) return;
    const idx = list.querySelectorAll('.inv-item-row').length;
    const row = document.createElement('div');
    row.className = 'form-row inv-item-row';
    row.style.cssText = 'align-items:flex-end;margin-bottom:8px;';
    row.innerHTML = `
      <div class="form-group" style="flex:3;"><input type="text" class="form-control" placeholder="Item description" data-inv-field="desc" data-inv-idx="${idx}"></div>
      <div class="form-group" style="flex:0 0 70px;"><input type="number" class="form-control" placeholder="Qty" value="1" min="1" data-inv-field="qty" data-inv-idx="${idx}" onchange="window.app.recalcInvoice()"></div>
      <div class="form-group" style="flex:0 0 100px;"><input type="number" class="form-control" placeholder="Rate" value="0" step="0.01" data-inv-field="rate" data-inv-idx="${idx}" onchange="window.app.recalcInvoice()"></div>
      <div class="form-group" style="flex:0 0 100px;"><input type="text" class="form-control" value="$0.00" readonly data-inv-field="subtotal" data-inv-idx="${idx}" style="background:var(--gray-50);font-weight:600;"></div>
      <button class="btn btn-sm" onclick="this.closest('.inv-item-row').remove();window.app.recalcInvoice();" style="color:var(--red);height:40px;">&times;</button>
    `;
  ...
  recalcInvoice() {
    ...
    document.querySelectorAll('.inv-item-row').forEach(row => {
      const qty = parseFloat(row.querySelector('[data-inv-field="qty"]')?.value) || 0;
      const rate = parseFloat(row.querySelector('[data-inv-field="rate"]')?.value) || 0;
      const sub = qty * rate;
      total += sub;
      const subEl = row.querySelector('[data-inv-field="subtotal"]');
  ...
  saveInvoice() {
    ...
    document.querySelectorAll('.inv-item-row').forEach(row => {
      items.push({
        description: row.querySelector('[data-inv-field="desc"]')?.value?.trim() || '',
        qty: parseFloat(row.querySelector('[data-inv-field="qty"]')?.value) || 1,
        rate: parseFloat(row.querySelector('[data-inv-field="rate"]')?.value) || 0,
      });
    });
    ...
    if (!data.clientName || !data.description) {
      showToast('Client and description are required');
      return;
    }

    if (id) {
      store.update('invoices', id, data);
      showToast('Invoice updated');
    } else {
      data.createdAt = new Date().toISOString();
      store.add('invoices', 'invoice', data);
  ...
  async markInvoicePaid(id) {
    if (!await appConfirm('Mark this invoice as paid?', { title: 'Mark Paid' })) return;
    store.update('invoices', id, { status: 'paid', paidAt: new Date().toISOString() });
    showToast('Invoice marked as paid');
    renderInvoicesPage();
  },
  viewInvoice(id) {
    const inv = (store.getAll('invoices') || []).find(i => i.id === id);
    if (!inv) return;
    const orgs = store.getAll('organizations');
    const org = orgs[0] || {};

    const modal = document.getElementById('log-modal');
    document.getElementById('log-modal-title').textContent = 'Invoice ' + (inv.invoiceNumber || '');
    document.getElementById('log-modal-body').innerHTML = `
      <div style="font-size:13px;" id="invoice-print-area">
        <div style="display:flex;justify-content:space-between;margin-bottom:20px;">
          <div>
            <div style="font-size:20px;font-weight:800;color:var(--brand-700);">${escHtml(org.name || 'EnnHealth')}</div>
            <div class="text-muted">${escHtml(org.phone || '')} | ${escHtml(org.email || '')}</div>
          </div>
          <div style="text-align:right;">
            <div style="font-size:18px;font-weight:700;">INVOICE</div>
            <div><strong>${escHtml(inv.invoiceNumber || '')}</strong></div>
            <div class="text-muted">Date: ${inv.invoiceDate || '—'}</div>
            <div class="text-muted">Due: ${inv.dueDate || '—'}</div>
          </div>
        </div>
        <div style="margin-bottom:16px;">
          <div style="font-weight:600;margin-bottom:2px;">Bill To:</div>
          <div>${escHtml(inv.clientName || '')}</div>
          ${inv.clientEmail ? `<div class="text-muted">${escHtml(inv.clientEmail)}</div>` : ''}
        </div>
        <table style="width:100%;margin-bottom:16px;">
          <thead><tr style="border-bottom:2px solid var(--gray-300);"><th style="text-align:left;padding:8px 0;">Description</th><th style="text-align:right;padding:8px 0;">Qty</th><th style="text-align:right;padding:8px 0;">Rate</th><th style="text-align:right;padding:8px 0;">Amount</th></tr></thead>
          <tbody>
            ${(inv.items || []).map(item => `
              <tr style="border-bottom:1px solid var(--gray-200);">
                <td style="padding:8px 0;">${escHtml(item.description || '')}</td>
                <td style="text-align:right;padding:8px 0;">${item.qty}</td>
                <td style="text-align:right;padding:8px 0;">$${(item.rate || 0).toFixed(2)}</td>
                <td style="text-align:right;padding:8px 0;font-weight:600;">$${((item.qty || 0) * (item.rate || 0)).toFixed(2)}</td>
              </tr>
            `).join('')}
          </tbody>
        </table>
        <div style="text-align:right;padding:12px;background:var(--gray-50);border-radius:var(--radius-lg);">
          <div style="font-size:20px;font-weight:800;">Total: $${parseFloat(inv.total || 0).toFixed(2)}</div>
          <div><span class="badge badge-${inv.status === 'paid' ? 'approved' : inv.status === 'overdue' ? 'denied' : 'submitted'}">${escHtml(inv.status || 'draft')}</span></div>
        </div>
        ${inv.notes ? `<div style="margin-top:16px;padding:12px;background:var(--gray-50);border-radius:var(--radius-lg);"><div style="font-weight:600;margin-bottom:4px;">Notes:</div><div class="text-muted">${escHtml(inv.notes)}</div></div>` : ''}
      </div>
      <div style="margin-top:16px;display:flex;gap:10px;justify-content:flex-end;">
        <button class="btn btn-sm" onclick="window.print()">Print</button>
        <button class="btn btn-sm" onclick="window.app.emailInvoice('${inv.id}')">Email to Client</button>
        <button class="btn" onclick="window.closeLogModal()">Close</button>
      </div>
    `;
    modal.classList.add('active');
  },
  emailInvoice(id) {
    ...
    // Pre-fill the Send Email page
    window.closeLogModal();
    navigateTo('send-email');
    setTimeout(() => {
      const toEl = document.getElementById('email-to');
      const subjectEl = document.getElementById('email-subject');
      const bodyEl = document.getElementById('email-body');
      if (toEl && inv.clientEmail) toEl.value = inv.clientEmail;
      if (subjectEl) subjectEl.value = `Invoice ${inv.invoiceNumber} from ${orgName} — $${parseFloat(inv.total || 0).toFixed(2)}`;
      if (bodyEl) bodyEl.value = `Dear ${inv.clientName || 'Client'},\n\nPlease find below the details for Invoice ${inv.invoiceNumber}.\n\nAmount Due: $${parseFloat(inv.total || 0).toFixed(2)}\nDue Date: ${inv.dueDate || 'N/A'}\nDescription: ${inv.description || ''}\n\n${(inv.items || []).map(i => `• ${i.description}: ${i.qty} x $${(i.rate || 0).toFixed(2)} = $${((i.qty || 0) * (i.rate || 0)).toFixed(2)}`).join('\n')}\n\n${inv.notes || ''}\n\nThank you for your business.\n\nBest regards,\n${orgName}`;
    }, 100);
  },
};
...
function openApplicationModal(id) {
  ...
  const existing = id ? store.getById('applications', id) : null;
  title.textContent = existing ? 'Edit Application' : 'Add Application';

  const licensedStates = store.getAll('licenses').map(l => l.state);
  const providers = store.getAll('providers');
  ...
  form.innerHTML = `
    ...
    <div class="form-row">
      <div class="form-group"><label>Submitted Date</label><input type="date" class="form-control" id="field-submitted" value="${toDateInputValue(existing?.submittedDate)}"></div>
      <div class="form-group"><label>Effective Date</label><input type="date" class="form-control" id="field-effective" value="${toDateInputValue(existing?.effectiveDate)}"></div>
    </div>
    <div class="form-row">
...
window.saveApplication = async function() {
  ...
  if (id) {
    const existing = store.getById('applications', id);
    if (existing && existing.status !== data.status) {
      ...
      store.add('activity_logs', 'activity_log', {
        applicationId: id,
        type: 'status_change',
    ...
    store.update('applications', id, data);
    showToast('Application updated');
  } else {
    const result = store.add('applications', 'application', data);
    if (result.success) {
      // Auto-log creation
      store.add('activity_logs', 'activity_log', {
...
function openLicenseModal(id) {
  ...
  const existing = id ? store.getById('licenses', id) : null;
  ...
  form.innerHTML = `
    ...
    <div class="form-row">
      <div class="form-group"><label>Issue Date</label><input type="date" class="form-control" id="lic-issued" value="${toDateInputValue(existing?.issueDate)}"></div>
      <div class="form-group"><label>Expiration Date</label><input type="date" class="form-control" id="lic-expiration" value="${toDateInputValue(existing?.expirationDate)}"></div>
    </div>
...
window.saveLicense = function() {
  ...
  if (id) {
    store.update('licenses', id, data);
    showToast('License updated');
  } else {
    store.add('licenses', 'license', data);
...
window.saveActivityLog = function() {
  ...
  if (contactName || contactPhone) {
    ...
    if (app) {
      ...
      store.update('applications', applicationId, updates);
    }
  }
...
function viewActivityLog(applicationId) {
  const modal = document.getElementById('log-modal');
...
function getAlerts() {
  ...
  const upcomingFU = getUpcomingFollowups(3);
...
function updateNotificationBell() {
  let alerts;
  try { alerts = getAlerts(); } catch (e) { console.warn('getAlerts() error:', e); alerts = []; }
...
function renderApplicationTimeline(appId) {
  const app = store.getById('applications', appId);
  if (!app) return;
  ...
  const tasks = store.getAll('tasks').filter(t => t.linkedAppId === appId);
...
function renderDocumentTracker() {
  const body = document.getElementById('page-body');
  const apps = store.getAll('applications');
...
function renderReimbursement() {
  ...
  const pPlans = store.getAll('payer_plans') || [];
  pPlans.forEach(plan => {
...
function renderCredentialingRenewalSection(today) {
  const allApps = store.getAll('applications').filter(a => a.status !== 'denied' && a.status !== 'withdrawn');
  if (allApps.length === 0) {
    return `
  ...
  body.innerHTML = `
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:12px;">
      <div class="form-group">
        ...
        <input type="date" id="edit-task-due" class="form-control" value="${toDateInputValue(task.dueDate)}">
      </div>
      <div class="form-group">
